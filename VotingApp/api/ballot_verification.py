...
import base64
import json
from coloursVA import GREEN, ORANGE, YELLOW, PINK, BOLD
import fetch_functions_va as ff

async def verify_ballot(election_id, voter_id, timestamp):
    ballot: Ballot = await ff.fetch_ballot_from_bb(election_id, voter_id, timestamp)

    return verify_proof(election_id, voter_id, ballot)
